...
# Uses beem + Binance spot HIVE/USDT as external reference price
...
# Fetch Binance HIVE/USDT spot price as external reference
print("\n--- Fetching external reference price from Binance ---")
try:
    ...
    hive_usdt = float(response.json()["price"])
    reference_price = hive_usdt
    print(f"Binance HIVE/USDT : {hive_usdt:.6f} USDT")
...
print(f"\nReference price used for slippage: {reference_price:.6f} USDT/HIVE")

# Show premium/discount of internal market vs Binance
if is_buy:
    premium_ask_pct = ((internal_lowest_ask / reference_price) - 1) * 100
    print(f"Internal lowest ask premium vs Binance: {premium_ask_pct:+.4f}%")
else:
    premium_bid_pct = ((internal_highest_bid / reference_price) - 1) * 100
    print(f"Internal highest bid vs Binance: {premium_bid_pct:+.4f}%")

# Get order book
orderbook = market.orderbook(limit=orderbook_limit, raw_data=True)

# === Print TOP 20 BIDS + TOP 20 ASKS for full depth context ===
print("\n" + "="*70)
...
print(f"\n--- Slippage Simulation: {sim_title} vs Binance ({reference_price:.6f}) ---")
...
print(f"\n>>> Total liquidity within {tolerable_slip_pct}% slippage vs Binance: {cumulative_hbd_liquidity:.3f} HBD (≈USDT) {liquidity_desc} {cumulative_hive:.3f} HIVE <<<")

# Final results
if cumulative_hive > 0:
    avg_price = cumulative_hbd / cumulative_hive
    slippage_pct = ((avg_price / reference_price) - 1) * 100
    print(f"\n=== Results ({action.upper()}ING HIVE, staying within {tolerable_slip_pct}% slippage vs Binance) ===")
    ...
    print(f"Binance reference     : {reference_price:.6f} USDT per HIVE")
    ...
    print("\nNo liquidity available within the slippage tolerance (internal prices too far from Binance).")
